...
// Thrown when an identity, generation or enum encoding is malformed. Identities
// are untrusted input on every decode path.
class PATH_AUTHORITY_API IdentityError : public std::invalid_argument {
 public:
  explicit IdentityError(const std::string& what) : std::invalid_argument(what) {}
};

namespace detail {
PATH_AUTHORITY_API bool valid_identity_text(std::string_view text, std::size_t max_len) noexcept;
}  // namespace detail

// Strongly typed, bounded, self-validating text identity. Distinct Tag types
// are distinct C++ types and never convert into one another.
...
class StringId {
 public:
  ...
  static StringId parse(std::string_view text) {
    ...
    if (!parsed.has_value()) {
      throw IdentityError("malformed " + std::string(type_name()) + " encoding");
    }
    return *parsed;
  }
...
class Generation {
 public:
  ...
  // Callers must check can_advance() first; generation overflow is a defect.
